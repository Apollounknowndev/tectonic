...
public record OverlayConditionsMetadata(List<Entry> overlays) {
    ...
    public static final MetadataSectionType<OverlayConditionsMetadata> SERIALIZER = MetadataSectionType.fromCodec("neoforge:overlays", CODEC);

    public List<String> appliedOverlays() {
        List<String> appliedOverlays = new ArrayList<>();

        for (Entry entry : this.overlays()) {
            if (entry.condition().test(ICondition.IContext.TAGS_INVALID)) {
    ...
    public record Entry(String directory, ICondition condition) {
        public static final Codec<Entry> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            ICondition.CODEC.fieldOf("condition").forGetter(Entry::condition)
